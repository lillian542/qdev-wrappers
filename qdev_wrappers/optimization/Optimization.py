...
class Optimization:
    ...
    def __init__(self, method, start=None):
        self.method = method
        self.params = method.params
        self.num_attempts = 0

        if method.from_saved_data:
            self.start = start
        else:
    ...
    def optimize(self, *params):
        ...
        setpoints = []
        if self.method.from_saved_data:
        ...
        with meas.run() as datasaver:
            ...
            res = [(param, val) for param, val in self.current.items()]
    ...
    #         best_value.append(get_measured_data(params, self.method.measured_params, best))
    #
    #         if success_condition is not None:
